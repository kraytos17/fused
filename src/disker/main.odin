...
main :: proc() {
	...
	for i in 0 ..< int(reserved_clusters) {
		map_table[i] = fs.Cluster_Map_Entry{flags = fs.Cluster_Map_Flags{.Reserved, .Full}}
	}
	...
	ce_table[0] = fs.Cluster_Entry{state = fs.Cluster_Entry_State{.Allocated, .Cluster_Map}, allocation_size = 1, sector_start = 0}
	ce_table[1] = fs.Cluster_Entry{state = fs.Cluster_Entry_State{.Allocated, .Directory},   allocation_size = 1, sector_start = 1}
	if embed_demo {
