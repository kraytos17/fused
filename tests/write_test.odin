...
// without requiring a FUSE mount.  Uses shared open_test_volume.
#+build linux
package tests
...
test_write_append :: proc(t: ^testing.T) {
	...
	runs2, runs_ok2 := fs.resolve_extents(&vol, fc, fo)
...
test_directory_entry_persistence :: proc(t: ^testing.T) {
	...
	rd_ce, ok_ce := fs.find_cluster_entry(&vol, root_cluster, root_offset)
...
test_directory_growth :: proc(t: ^testing.T) {
	...
	for i := 0; i < 11; i += 1 {
		...
		dir_runs, dr_ok := fs.resolve_extents(&vol, root_cluster, root_offset)
	...
	dirs, dirs_ok := fs.read_directory_entries(&vol, root_cluster, fs.Sector_Offset(rd_ce.sector_start))
	defer delete(dirs)
	...
	dir_runs2, dr2_ok := fs.resolve_extents(&vol, root_cluster, root_offset)
...
test_entry_timestamp :: proc(t: ^testing.T) {
	...
	ce, ce_ok := fs.find_cluster_entry(&vol, dir_cluster, dir_offset)
...
test_dir_entry_create_delete_recreate :: proc(t: ^testing.T) {
	...
	root_cluster := fs.Cluster(vol.master.root_cluster)
	root_offset := fs.Sector_Offset(vol.master.root_sector_index)
	rd_ce, ce_ok := fs.find_cluster_entry(&vol, root_cluster, root_offset)
...
test_rename_overwrite_simulation :: proc(t: ^testing.T) {
	...
	ce, ce_ok := fs.find_cluster_entry(&vol, root_c, root_o)
...
test_cross_dir_rename_simulation :: proc(t: ^testing.T) {
	...
	rce, rce_ok := fs.find_cluster_entry(&vol, root_c, root_o)
	...
	ace, ace_ok := fs.find_cluster_entry(&vol, ac, ao)
	...
	bce, bce_ok := fs.find_cluster_entry(&vol, bc, bo)
...
test_truncate_to_zero :: proc(t: ^testing.T) {
	...
	_, rok := fs.resolve_extents(&vol, fc, fo)
...
test_truncate_partial :: proc(t: ^testing.T) {
	...
	ce, ce_ok := fs.find_cluster_entry(&vol, fc, fo)
	...
	for &e, _ in ce_tbl {
		if e.sector_start == u16(fo) {
			e = ce
			testing.expect(t, fs.write_cluster_entry_table(&vol, fc, &ce_tbl), "write partial CE")
			break
		}
	}
...
test_lfn_create_and_read :: proc(t: ^testing.T) {
	...
	rce, rce_ok := fs.find_cluster_entry(&vol, fs.Cluster(vol.master.root_cluster), fs.Sector_Offset(vol.master.root_sector_index))
	...
	dirs, dirs_ok := fs.read_directory_entries(&vol, fs.Cluster(vol.master.root_cluster), fs.Sector_Offset(vol.master.root_sector_index))
	defer delete(dirs)
...
test_symlink_create_and_read :: proc(t: ^testing.T) {
	...
	runs, rok := fs.resolve_extents(&vol, new_c, new_o)
	...
	for &d in dirs {
		if .Link in d.flags {
			...
			runs2, rok2 := fs.resolve_extents(&vol, fs.Cluster(d.stored_cluster), fs.Sector_Offset(d.sector_index))
...
test_fallocate_extend :: proc(t: ^testing.T) {
	...
	runs2, rok2 := fs.resolve_extents(&vol, fc2, fo2)
...
test_resolve_lfn_actual_resolution :: proc(t: ^testing.T) {
	...
	lfn_runs, runs_ok := fs.resolve_extents(&vol, lfn_c, lfn_o)
	...
	lfn_entry, found := fs.find_cluster_entry(&vol, lfn_c, lfn_o)
